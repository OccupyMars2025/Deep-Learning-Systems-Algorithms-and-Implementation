...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    for i in range(len(args)):
        for j in range(args[i].realize_cached_data().size):
            # Caution: 
            # 1. If args[i].realize_cached_data().dtype is an integer type,
            # the following operation will have no effect or unexpected effects
            # 2. You had better use "float64" as the dtype, or you may get a big enough error
            # to fail the check
...
# TODO: In the matmul case, (f1 - f2) / (2 * eps) in the "gradient_check" is more interesting.
#  It needs some math to clarify it.
...
def test_broadcast_to_backward():
    """
    In this case, use (f1 - f2)/ (2*epsilon) to get the approximate gradient w.r.t each element,
...
def test_summation_backward():
    gradient_check(ndl.summation, ndl.Tensor(np.random.randn(5,4)), axes=(1,), keep_axes=True)
    gradient_check(ndl.summation, ndl.Tensor(np.random.randn(5,4)), axes=(0,))
    gradient_check(ndl.summation, ndl.Tensor(np.random.randn(5,4)), axes=(0,1), keep_axes=True)
...
def test_nn_epoch_ndl():
    ...
    np.testing.assert_allclose(ndl.relu(ndl.Tensor([[-46.9 , -48.8 , -45.45, -49.  ],
       ...
       [0., 0., 0., 0.],
       [0., 0., 49.3, 0.]]))
    a111 = np.random.randint(-100, 100, (4, 5))
    a111[a111 == 0] = 1  # so there are no zero elements in a111
    gradient_check(ndl.relu, ndl.Tensor(a111.astype("float64")))
    # gradient_check(ndl.relu, ndl.Tensor(np.random.randint(-100, 100, (4, 5)).astype("float64"))) # pass the check
    # gradient_check(ndl.relu, ndl.Tensor(np.random.randint(-100, 100, (4, 5)).astype("float32"))) # fail the check
    # gradient_check(ndl.relu, ndl.Tensor(np.random.randint(-100, 100, (4, 5))))                   # fail the check
